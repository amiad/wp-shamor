...
class Shamor
{
	...
	private $http_status, $location, $country, $weekday;
	...
	function get_shabbat_times()
	{
		...
		$this->weekday = $dt->format('l');

		$sunset = date_sun_info(strtotime($this->weekday), $this->location->latitude, $this->location->longitude)['sunset'];
	...
	{
		$juldate = gregoriantojd(...explode('/', date('m/d/Y', strtotime($this->weekday . " $str"))));
	...
	function move_out_of_site($template = false)
	{
		...
		$shamor_enable_block = defined('SHAMOR_ENABLE_BLOCK') && SHAMOR_ENABLE_BLOCK;

		if (
			!empty($_GET['wp_shamor'])
			|| (!$times)
			|| (($this->weekday == 'Friday' || $this->is_erev_yom_tov()) && time() > $times['candle_lighting'])
			|| (($this->weekday == 'Saturday' || $this->is_yom_tov()) && time() < $times['havdalah'])
			|| $shamor_enable_block
		) {
			if (wp_doing_ajax()) {
				return true;
